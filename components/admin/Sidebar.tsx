...
export const AdminSidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, onLogout }) => {
    const menuItems = [
        ...
        { id: 'requests', label: 'Bookings', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
        { id: 'destinations', label: 'Destinations', icon: 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z' },
        { id: 'blogs', label: 'Blog & Articles', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
    ...
    return (
        <aside className="w-64 bg-charcoal-900 border-r border-white/5 flex flex-col h-full bg-gradient-to-b from-charcoal-900 to-charcoal-950">
            <div className="p-6 flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-forest-500 flex items-center justify-center text-white font-bold text-lg">
                    TM
                </div>
                <div>
                    <h1 className="font-display font-bold text-lg text-white leading-none">TravelMate</h1>
                    <span className="text-xs text-charcoal-400 font-medium">Admin Panel</span>
                </div>
            </div>
                ...
                {menuItems.map((item) => (
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group ${activeTab === item.id
                            ? 'bg-forest-600/20 text-forest-400 border border-forest-500/20 shadow-lg shadow-forest-900/20'
                            : 'text-charcoal-400 hover:text-white hover:bg-white/5'
                            }`}
                    >
                        <svg
                            className={`w-5 h-5 transition-colors ${activeTab === item.id ? 'text-forest-400' : 'text-charcoal-500 group-hover:text-white'}`}
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth={2}
                        ...
                        {item.label}
                        {activeTab === item.id && (
                            <div className="ml-auto w-1.5 h-1.5 rounded-full bg-forest-400"></div>
                        )}
                    </button>
                ))}
            </nav>
